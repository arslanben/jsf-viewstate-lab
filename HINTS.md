...
3. The server has to turn that blob back into objects on every postback. What does that
   imply for a value the client can replace entirely, and when in the request lifecycle
   does that conversion happen?

4. Enumerate the libraries shipped with the application (the `WEB-INF/lib` directory, error
   pages, dependency fingerprints). One of them is a classic gadget dependency.

5. The chain does not need a known-vulnerable framework class; it needs a class that can
...
6. The container has no outbound connectivity, so reverse shells and DNS callbacks will not
